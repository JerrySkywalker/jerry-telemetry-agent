...
$gatewayProcess = $null; $mockProcess = $null
...
try {
  ...
  Set-Content -LiteralPath $secretPath -Value $fixtureSecret -NoNewline
  ...
  $nodeBase | ConvertTo-Json -Depth 10 | Set-Content -LiteralPath $disabledNode -Encoding UTF8
  $nodeBase.collectors[0].enabled = $true
  $nodeBase | ConvertTo-Json -Depth 10 | Set-Content -LiteralPath $enabledNode -Encoding UTF8
  ...
  ) | Set-Content -LiteralPath $disabledEnv -Encoding UTF8
  ...
  $disabledProcess.WaitForExit(30000) | Out-Null
  Assert-True ($disabledProcess.HasExited -and $disabledProcess.ExitCode -eq 0) "agent_disabled_artifact_boot_failed"
...
'@ | Set-Content -LiteralPath $mockScript -Encoding UTF8
  ...
  @(
    ...
    "SPOOL_DIR=$stateRoot\spool", "AGENT_INTERVAL_SECONDS=1", "HEALTH_SERVER_ENABLED=true",
    "HEALTH_HOST=127.0.0.1", "HEALTH_PORT=$agentHealthPort"
  ) | Set-Content -LiteralPath $enabledEnv -Encoding UTF8
  ...
  $enabledProcess.WaitForExit(30000) | Out-Null
  Assert-True ($enabledProcess.HasExited -and $enabledProcess.ExitCode -eq 0) "agent_enabled_artifact_boot_failed"
  ...
  Stop-FixtureProcess $mockProcess; Stop-FixtureProcess $gatewayProcess
