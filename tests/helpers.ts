import os from "node:os";
import path from "node:path";
import { mkdtemp } from "node:fs/promises";
import type { Config } from "../src/config.js";

export async function tempDir(): Promise<string> {
  return mkdtemp(path.join(os.tmpdir(), "jta-"));
}

export function testConfig(overrides: Partial<Config> = {}): Config {
  return {
    mode: "once",
    dryRun: false,
    provider: "backend-usage",
    outputModes: ["stdout"],
    intervalSeconds: 600,
    codexHome: "/home/test/.codex",
    codexUsageEndpoint: "https://chatgpt.com/backend-api/wham/usage",
    codexStatusLatestPath: "/input/latest.json",
    hostCodexBin: "/host-bin/codex",
    hostCodexHome: "/host-codex-home",
    codexTmuxSession: "codex-status-agent",
    hubUrl: "http://127.0.0.1:0/telemetry",
    hubBatchUrl: "",
    hubRequestTimeoutMs: 5000,
    nodeId: "us-lax-pro-01",
    nodeSecret: "test-secret",
    nodeKeyId: "",
    hostname: "novix-lax-01",
    region: "us-lax",
    collector: "codex-backend-usage",
    collectorMode: "codex-backend-usage",
    collectorConfigs: [{ name: "codex-backend-usage", enabled: true, interval_seconds: 600 }],
    nodeConfigPath: "",
    accountLabel: "",
    nodeRole: "",
    platform: "linux",
    outputFile: "/state/codex-usage-latest.safe.snapshot.json",
    usageLatestPath: "/state/codex-usage-latest.safe.snapshot.json",
    usageLastGoodPath: "/state/codex-usage-last-good.safe.snapshot.json",
    serverBatchLatestFile: "/state/server-batch-latest.safe.json",
    serverBatchOutputFile: "/state/server-batch.safe.json",
    statePath: "/state/agent-state.json",
    spoolDir: "/state/spool",
    spoolMaxFiles: 100,
    spoolMaxBytes: 10 * 1024 * 1024,
    forceSend: false,
    healthServerEnabled: false,
    healthHost: "127.0.0.1",
    healthPort: 8081,
    agentHealthEnabled: false,
    agentHealthEventType: "telemetry.agent.health",
    agentHealthIntervalSeconds: 600,
    agentHealthOutputFile: "/state/agent-health-latest.safe.snapshot.json",
    ...overrides
  };
}
